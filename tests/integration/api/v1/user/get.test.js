...
describe("GET /api/v1/user", () => {
  describe("Anonymous user", () => {
    test("Retrieving the endpoint", async () => {
      ...
      expect(responseBody).toEqual({
        ...
        action: "Verifique se o usuário tem a feature: read:session",
        statusCode: 403,
      });
    });
  });
  ...
  describe("Default user", () => {
    ...
    test("With nonexistent session", async () => {
      const nonextentToken =
        "4d1a82aee5c730dc8490788cf79a99df6a82a700fe5c314f5ccd4f5ffaebd37e1da1b07157794f4a938c28257475e511";

      const response = await fetch("http://localhost:3000/api/v1/user", {
        headers: {
          Cookie: `session_id=${nonextentToken}`,
        },
      });
    ...
    test("With valid session and expirate_at in half time", async () => {
      jest.useFakeTimers({
        now: new Date(Date.now() - session.EXPIRATION_IN_MILLISECONDS / 2),
      });
      const createdUser = await orchestrator.createUser({
        username: "UserWithValidHalfTimeSession",
      });

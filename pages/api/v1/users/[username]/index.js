...
async function patchHandler(request, response) {
  ...
  userTryingToPatch.features.push("read:user:email");

  const secureteOutputValues = authorization.filterOutput(
    userTryingToPatch,
    "read:user",
    updatedUser,
  );
  return response.status(200).json(secureteOutputValues);
}
